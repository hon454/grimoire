...
3. Change request에 `<!-- merge-readiness-sync -->` marker가 있는 댓글이 있으면 갱신하고, 없으면 새 댓글로 이슈를 만든 이유와 링크를 남긴다.
